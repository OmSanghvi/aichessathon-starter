...
for k,t in pst.items():
    p=lambda r:(0,)+tuple(x+piece[k] for x in r)+(0,)
    pst[k]=sum((p(t[i*8:i*8+8]) for i in range(8)),())
    pst[k]=(0,)*20+pst[k]+(0,)*20

K_MID=pst["K"]
K_END=tuple(piece["K"]+70-10*(abs(2*(i//10)-11)+abs(2*(i%10)-9)) for i in range(120))
...
class Position(namedtuple("Position","board score wc bc ep kp")):
            ...
            for d in directions[p]:
                for j in count(i+d,d):
                    ...
                    if p=="P":
                        ...
                        if A8<=j<=H8:
                            yield from(Move(i,j,x)for x in"NBRQ")
                            break
                    yield Move(i,j,"")
    ...
    def rotate(self,nullmove=False):
        return Position(self.board[::-1].swapcase(),-self.score,self.bc,self.wc,119-self.ep if self.ep and not nullmove else 0,119-self.kp if self.kp and not nullmove else 0)

    def move(self,m):
        i,j,prom=m
        put=lambda b,i,p:b[:i]+p+b[i+1:]
        p,q,b,wc,bc,ep,kp=self.board[i],self.board[j],self.board,self.wc,self.bc,0,0
        ...
        if p=="K":
            ...
            if abs(j-i)==2:
                kp=(i+j)//2
                b=put(b,A1 if j<i else H1,".")
                b=put(b,kp,"R")
        if p=="P":
            if A8<=j<=H8:b=put(b,j,prom)
            if j-i==2*N:ep=i+N
            if j==self.ep:b=put(b,j+S,".")
        return Position(b,score,wc,bc,ep,kp).rotate()
...
def board_to_sunfish(b):
    ...
    for i,p in enumerate(x):
        if p in"PNBRQK":score+=pst[p][i]
        elif p in"pnbrqk":score-=pst[p.upper()][119-i]
    ...
    p=Position(x,score,wc,bc,ep,0)
    return p if b.turn==chess.WHITE else p.rotate()

def sunfish_move_to_uci(m,b):
    i,j,p=m
    if b.turn==chess.BLACK:i,j=119-i,119-j
    u=chess.square_name(sunfish_to_square(i))+chess.square_name(sunfish_to_square(j))
    return u+(p.lower()if p else"")
...
def get_move(fen:str,time_left_ms:int)->str:
    ...
    if not legal:return""
    p=board_to_sunfish(b)
    t=max(time_left_ms,1)/1000
    searcher.deadline=time.time()+min(2,max(.02,t*.08))
    searcher.soft=searcher.deadline
    best=None
    try:
        for _,_,_,m in searcher.search([p]):
            if m:best=m
    except Stop:
        pass
    if best:
        u=sunfish_move_to_uci(best,b)
        try:
            if chess.Move.from_uci(u)in b.legal_moves:return u
        except ValueError:
            pass
    return legal[0].uci()